...
export default function LocationSearch({
  ...
  return (
    <div ref={containerRef} className={className} style={{ position: "relative" } as React.CSSProperties}>
      <div
        style={{
          display: "flex",
          alignItems: "center",
      ...
      >
        <Search size={15} style={{ color: "var(--text-tertiary)", flexShrink: 0 } as React.CSSProperties} />
        <input
          type="text"
          value={query}
        ...
        {searching && (
          <div
            style={{
              ...
              animation: "loc-search-spin 0.5s linear infinite",
            }}
          />
        )}
        {query && !searching && (
          <button onClick={clearSelection} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex" }}>
            <X size={14} style={{ color: "var(--text-tertiary)" } as React.CSSProperties} />
          </button>
        )}
      </div>
        ...
        >
          {results.map((item, i) => {
            ...
            return (
              ...
              >
                <MapPin size={14} style={{ color: "var(--amber)", flexShrink: 0, marginTop: "0.15rem" } as React.CSSProperties} />
                <div>
                  <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text-primary)" } as React.CSSProperties}>
                    {[area, town].filter(Boolean).join(", ") || item.display_name.split(",")[0]}
                  </div>
                  {county && (
                    <div style={{ fontSize: "0.7rem", color: "var(--text-tertiary)", marginTop: "0.1rem" } as React.CSSProperties}>
                      {county} County
                    </div>
                  )}
                  {item.type && (
                    <div style={{ fontSize: "0.65rem", color: "var(--text-tertiary)", marginTop: "0.05rem", textTransform: "capitalize" } as React.CSSProperties}>
