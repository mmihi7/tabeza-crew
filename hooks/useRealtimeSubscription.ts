 ...
 *
 * Handles:
 *   - Debounced handlers (prevents rapid re-renders)
 *   - Automatic retry on CHANNEL_ERROR / TIMED_OUT
 *   - Cleanup on unmount
 *   - Stale-closure-safe handler invocation via configsRef
 *
 * Usage:
 *   const { isConnected } = useRealtimeSubscription(
 *     [{ channelName: 'crew-notif-me', table: 'crew_notifications',
 *        filter: 'crew_member_id=eq.xxx', handler: (p) => ... }],
 *     [crewMemberId]
 *   )
 */
